...
经过这一系列操作，我们把一个(可能很长)的临界区转换为了 **较短** 的临界区 —— 用一个很短的临界区来判断线程是否成功获取锁，在线程获取锁失败后，线程不再空转，而是跑去干别的了(或者进入睡眠了) ，此时临界区就暂时结束了 (这里可能讲的不是很清楚，只能意会了)。具体内容详见 [Lec 07 - mutex(2)](https://www.bilibili.com/video/BV1d2421A7uh) 视频中的 1:20:00 - 1:24:26 处。
